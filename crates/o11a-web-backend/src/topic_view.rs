...
pub fn render_source_text(
  ...
  // Global builtins
  if let Some(global) = crate::solidity_formatter::global_to_source_text(topic)
  {
    return Some(global);
  }
  ...
  match audit_data.nodes.get(topic) {
    Some(Node::Solidity(solidity_node)) => {
      Some(crate::solidity_formatter::node_to_source_text(
        solidity_node,
        &audit_data.nodes,
    ...
    Some(Node::Comment(nodes)) => {
      Some(crate::comment_formatter::render_comment_html(
        nodes,
        topic,
        &audit_data.nodes,
      ))
...
fn get_delimiter_opening(
  ...
  match audit_data.nodes.get(topic) {
    Some(Node::Solidity(node)) => {
      match crate::solidity_formatter::node_to_delimiter(
        node,
        &audit_data.nodes,
        &audit_data.topic_metadata,
      ) {
...
fn get_delimiter_closing(
  ...
  match audit_data.nodes.get(topic) {
    Some(Node::Solidity(node)) => crate::solidity_formatter::node_to_delimiter(
      node,
      &audit_data.nodes,
      &audit_data.topic_metadata,
    )
...
pub fn build_topic_view(
  ...
  audit_data: &AuditData,
  source_text_cache: &mut std::collections::HashMap<String, String>,
  cached: Option<&crate::state::CachedTopicView>,
