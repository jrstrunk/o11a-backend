...
**Conditions are positive assertions about a non-pure subject's interaction surface.** Each non-pure subject (state read/write, external call, delegatecall, assembly block, etc.) carries conditions — assertions about what its caller, inputs, callees, or surrounding state must look like for the subject's functional purpose and placement rationale to hold. Each condition carries a `kind` naming its category (see the closed `ConditionKind` enum in SPEC.md). Conditions provide concrete, bounded inputs from which threats are derived as adversarial inversions, rather than relying on open-ended threat identification.

**Threats live on non-pure source code subjects.** Each threat is the adversarial inversion of a specific condition — a concrete scenario in which the named assertion fails to hold ("the deterministic token address can be pre-computed and `createPair` called first, bricking deployment"). A threat names exactly one `falsifies_condition`; one condition can be the target of many threats. Each threat carries a structured `controlled_by` field naming the primary actor whose action drives the scenario, drawn from a closed eight-variant enum (`Caller`, `PrivilegedRole`, `External`, `BlockProducer`, `Counterparty`, `Self_`, `AnyParty`, `Other`). Threat descriptions stay actor-agnostic — actor identity lives in the structured field. Pure operations (arithmetic, comparisons, local assignments) are excluded because their threat surface is fully covered by type convergences.
...
**System characteristics capture system-wide developer-asserted ground truth.** System characteristics are audit-wide claims — security properties, trust assumptions, role definitions, threat-model statements — that an auditor must take as developer-asserted ground truth when reasoning about adversarial scenarios. They are extracted from documentation alongside requirements and then re-synthesized after feature synthesis using the raw `security.md` notes as additional context. Each characteristic carries a `kind` (currently only `Security`); the complete set of a given kind is consumed in entirety by one downstream pipeline step (Security → threats), replacing the role the raw `security.md` blob used to play in threat prompting.

**Every property requires approval, and approval requires a comment.** Properties in the security model — including features, requirements, behaviors, system characteristics, semantics, purpose, placement, conditions, threats, and invariants — transition from unverified to verified through explicit approval. Approval is bidirectional: human auditors approve AI-authored properties and AI agents approve human-authored properties. The required comment on each approval converts agreement from a frictionless click into a signal-producing act, capturing the reasoning that would otherwise stay implicit and counteracting the anchoring bias that lets well-written generated answers receive rubber-stamp acceptance. The collaborator module (in `crates/o11a-server`) implements the approval mechanism on the same comment surface used for general discussion.

### Initial Generation and On-the-Fly Generation

The security model is seeded by a ten-step automated pipeline: semantic linking, requirement extraction, behavior extraction, feature synthesis via reconciliation, characteristic synthesis, functional purpose & placement generation, condition generation, threat generation, invariant generation, and invariant validation. Invariants are defensive properties stated against each threat — what the codebase must enforce; the validation step produces a per-invariant verdict (`Enforced` / `Absent` / `Partial` / `Inconclusive`) on whether the property actually holds at the validated subject, with cross-site propagation deferred to a later pipeline step. The source-driven steps (behavior extraction through invariant validation) run only against in-scope contracts; the documentation-driven steps (requirements and characteristic synthesis) operate on the audit's documentation and `security.md`. Out-of-scope code and library dependencies are assumed correct, so the source-driven pipeline does not run against them, but they still receive lighter-weight semantics and behaviors so in-scope analysis has the context it needs. See `SPEC.md` for the per-step design.
