...
The security model is seeded by a nine-step automated pipeline: semantic linking, requirement extraction, behavior extraction, feature synthesis via reconciliation, characteristic synthesis, functional purpose & placement generation, condition generation, threat generation, and invariant generation. Invariants are defensive properties stated against each threat — what the codebase must enforce; verification of where they hold in the code is a deferred later pipeline step (re-check propagation). The source-driven steps (behavior extraction through invariant generation) run only against in-scope contracts; the documentation-driven steps (requirements and characteristic synthesis) operate on the audit's documentation and `security.md`. Out-of-scope code and library dependencies are assumed correct, so the source-driven pipeline does not run against them, but they still receive lighter-weight semantics and behaviors so in-scope analysis has the context it needs. See `SPEC.md` for the per-step design.
