...
The security model is seeded by an eight-step automated pipeline: semantic linking, requirement extraction, behavior extraction, feature synthesis via reconciliation, characteristic synthesis, functional purpose & placement generation, condition generation, and threat generation. The source-driven steps (behavior extraction through threat generation) run only against in-scope contracts; the documentation-driven steps (requirements and characteristic synthesis) operate on the audit's documentation and `security.md`. Out-of-scope code and library dependencies are assumed correct, so the source-driven pipeline does not run against them, but they still receive lighter-weight semantics and behaviors so in-scope analysis has the context it needs. See `SPEC.md` for the per-step design.
