    ...
    if let Some(in_scope_topic_declaration) = in_scope_topic_declaration {
      ...
      if let Some((kind, condition)) = control_flow_metadata {
        ...
        let kind = match node {
          ...
          // Call purity is populated during the transform phase from the
          // callee's stateMutability. The node field is the authoritative source.
          ASTNode::FunctionCall { call_purity, .. } => {
            UnnamedTopicKind::FunctionCall(*call_purity)
          }
...
@return amount Amount of tokens rescued";
  ...
  #[test]
  fn call_purity_default_is_non_pure() {
    // A FunctionCall with no resolvable callee retains the NonPure default
    // set during parsing.
    let call = make_function_call(
      ...
      ASTNode::Identifier {
        ...
        referenced_declaration: 999, // not resolvable
      },
    );
    if let ASTNode::FunctionCall { call_purity, .. } = call {
      assert!(
        matches!(call_purity, domain::CallKind::NonPure),
        "expected NonPure default, got {:?}",
        call_purity
      );
    } else {
